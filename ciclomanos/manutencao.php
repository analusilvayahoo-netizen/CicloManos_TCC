<?php

include 'config.php';

?>

<!DOCTYPE html>
<html lang="pt-br">
<head>
<meta charset="UTF-8">
<title>Manutenção - CicloManos</title>

<style>

*{
  margin:0;
  padding:0;
  box-sizing:border-box;
}

body{
  font-family: Arial;
  background:#f5f5f5;
}

a{
  text-decoration:none;
  color:inherit;
}

/* TOPO */
.topo{
  background:#f5f5f5;
  display:flex;
  justify-content:space-around;
  padding:8px;
  font-size:12px;
}

.topo a:hover{
  color:#e60000;
}

/* HEADER COMPLETO */
.meio-header{
  display:flex;
  align-items:center;
  justify-content:space-between;
  padding:15px 40px;
  background:white;
}

.logo{
  height:120px;
}

.busca{
  display:flex;
  width:40%;
}

.busca input{
  width:100%;
  padding:10px;
  border:1px solid #ccc;
  border-radius:5px 0 0 5px;
}

.busca button{
  padding:10px 20px;
  border:none;
  background:#4A86B8;
  color:white;
  border-radius:0 5px 5px 0;
  cursor:pointer;
}

.usuario a{
  display:block;
  margin:3px 0;
}

.usuario a:hover{
  color:#4A86B8;
}

/* MENU */
.menu{
  background:#4A86B8;
  color:white;
  display:flex;
  align-items:center;
  padding:10px 40px;
  gap:20px;
}

.menu nav a{
  margin:0 10px;
}

.menu nav a:hover{
  text-decoration:underline;
}

.departamentos{
  background:#3f719b;
  color:white;
  padding:10px 15px;
  border-radius:5px;
}

/* TITULO */
.titulo{
  text-align:center;
  margin:30px;
}

/* RASTREIO */
.rastreio{
  background:white;
  width:60%;
  margin:0 auto 40px;
  padding:30px;
  border-radius:12px;
  box-shadow:0 6px 15px rgba(0,0,0,0.1);
  text-align:center;
}

.rastreio input{
  width:60%;
  padding:12px;
  margin:10px 0;
  border-radius:5px;
  border:1px solid #ccc;
}

.rastreio button{
  padding:12px 25px;
  background:#4A86B8;
  color:white;
  border:none;
  border-radius:5px;
  cursor:pointer;
  transition:0.3s;
}

.rastreio button:hover{
  background:#3f719b;
}

/* STATUS */
.status{
  margin-top:30px;
  display:none;
}

.etapas{
  display:flex;
  justify-content:space-between;
  margin-top:20px;
}

.etapa{
  width:22%;
  padding:10px;
  border-radius:6px;
  background:#ddd;
}

.ativa{
  background:#4A86B8;
  color:white;
}

/* CAIXA DO JOGO */
.game-container{
  width:80%;
  margin:0 auto;
  background:white;
  border-radius:12px;
  box-shadow:0 6px 15px rgba(0,0,0,0.1);
  padding:20px;
}

/* GAME */
#game{
  position:relative;
  width:100%;
  height:300px;
  overflow:hidden;
  border-radius:10px;
}

#fundo{
  position:absolute;
  width:100%;
  height:100%;
  object-fit:cover;
  filter:brightness(0.8);
}

/* BIKE */
#bike{
  position:absolute;
  bottom:0;
  left:50px;
  width:80px;
  z-index:2;
}

/* PEDRA */
.pedra{
  position:absolute;
  bottom:0;
  width:40px;
  height:40px;
  object-fit:contain;
  z-index:2;
}

/* SCORE */
#score{
  position:absolute;
  top:10px;
  left:15px;
  font-size:18px;
  background:rgba(0,0,0,0.6);
  color:white;
  padding:5px 10px;
  border-radius:5px;
  z-index:3;
}

/* FOOTER */
footer{
  background:#111;
  color:white;
  text-align:center;
  padding:20px;
  margin-top:40px;
}

</style>
</head>

<body>

<!-- TOPO -->
<div class="topo">
  <a href="#">📍 Rastreie seu pedido</a>
  <a href="#">💬 Fale conosco</a>
  <span>📱 WhatsApp: (12) 99999-0000</span>
  <span>📞 Telefone: (12) 3721-0000</span>
</div>

<!-- HEADER IGUAL AO DE PEÇAS -->
<div class="meio-header">

  <a href="ciclomanos.html">
    <img src="https://i.pinimg.com/736x/88/99/99/889999c134977d6379c48cea6a4ff373.jpg" class="logo">
  </a>

  <div class="busca">
    <input type="text" placeholder="Digite o que você procura">
    <button>Buscar</button>
  </div>

  <div class="usuario">
    <a href="#">👤 Conta</a>
    <a href="#">🛒 Carrinho</a>
  </div>

</div>

<!-- MENU -->
<div class="menu">
  <a href="departamentos.html" class="departamentos">☰ Departamentos</a>

  <nav>
    <a href="acessorios.html">Acessórios</a>
    <a href="bicicletas.html">Bicicletas</a>
    <a href="pecas.html">Peças</a>
    <a href="manutencao.html">Manutenção</a>
    <a href="ofertas.html">Ofertas</a>
  </nav>
</div>

<h2 class="titulo">ACOMPANHE SUA MANUTENÇÃO</h2>

<!-- RASTREIO -->
<div class="rastreio">
  <p>Digite o código da sua manutenção:</p>
  <input type="text" id="codigo">
  <br>
  <button onclick="verificar()">Acompanhar</button>

  <div class="status" id="status">
    <h3>Status da Bicicleta:</h3>
    <div class="etapas">
      <div class="etapa" id="e1">Recebida</div>
      <div class="etapa" id="e2">Em análise</div>
      <div class="etapa" id="e3">Em manutenção</div>
      <div class="etapa" id="e4">Pronta</div>
    </div>
  </div>
</div>


<footer>
  <p>© 2026 CicloManos - Todos os direitos reservados</p>
</footer>

<script>

// RASTREIO
function verificar(){
  let codigo = document.getElementById("codigo").value;
  let status = document.getElementById("status");

  status.style.display = "block";

  document.querySelectorAll(".etapa").forEach(e => e.classList.remove("ativa"));

  if(codigo == "1"){
    e1.classList.add("ativa");
  }
  else if(codigo == "2"){
    e1.classList.add("ativa");
    e2.classList.add("ativa");
  }
  else if(codigo == "3"){
    e1.classList.add("ativa");
    e2.classList.add("ativa");
    e3.classList.add("ativa");
  }
  else{
    e1.classList.add("ativa");
    e2.classList.add("ativa");
    e3.classList.add("ativa");
    e4.classList.add("ativa");
  }
}


</script>

</body>
</html>